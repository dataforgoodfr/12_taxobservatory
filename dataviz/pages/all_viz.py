...
def show_all_viz():
    ...
    selected_company = st.selectbox(
        "Select a company", company_list, index=len(company_list) - 1
    )
    ...
    selected_sector = st.selectbox(
        "Select a sector", sector_list, index=len(sector_list) - 1
    )
    df_selected_sector = df[df["sector"] == selected_sector]

    country_list = list(df["jur_name"].unique())[::-1]
    selected_country = st.selectbox(
        "Select a country", country_list, index=len(country_list) - 1
    )
    ...
    def number_of_tracked_reports():
        ...
        row[1].write(
            ...
            + selected_company
            + ")"
            + str(number_of_tracked_reports_company)
        )
        row[1].write(
            "per sector (ex:"
            + selected_sector
            + ")"
            + str(number_of_tracked_reports_sector)
        )
        row[1].write(
            "per country (ex:"
            + selected_country
            + ") "
            + str(number_of_tracked_reports_country)
        )
        row[2].markdown(
            """
