...
def method_example(data, parameters):
    ...
    """
    results = {
        "tables": [pd.DataFrame([[10, 11], [12, 13]], columns=["A", "B"])],
        "pages": [1],
    ...
    return {
        "results": results,
        "img": convert_pdf_to_image(data["file"], dpi=parameters["dpi"]),
    }
...
def method_2(data, parameters):
    ...
    img = convert_pdf_to_image(data["file"], dpi=dpi)
    ...
    results = {"tables": [], "pages": [], "bounding_boxes": []}
...
def process_results(results: dict):
    ...
    if not isinstance(results, dict) or any(
        [key not in results.keys() for key in expected_keys]
    ):
    ...
    for idx, bbox in results["bounding_boxes"].items():
        if (
            not isinstance(bbox, dict) and len(bbox) == 4
        ):  # try to convert as x0, y0, x1, y1
            results["bounding_boxes"][idx] = {
                key: bbox[n] for n, key in enumerate(["x0", "y0", "x1", "y1"])
            }
        elif not isinstance(bbox, dict) or any(
            [key not in bbox.keys() for key in ["x0", "y0", "x1", "y1"]]
        ):
            raise ValueError(
                f"Error in formatting of 'bounding_boxes', expecting dict containing 'x0', 'y0', 'x1', 'y1' or a list of 4 values"
            )
    results["columns"] = {
        key: [{"id": c, "name": c, "hideable": True} for c in df.columns]
        for key, df in results["tables"].items()
    }
    results["records"] = {
        key: df.to_dict(orient="records") for key, df in results["tables"].items()
    }
