...
from dash_process_methods import DICT_OF_METHODS, DEFAULT_METHOD, DICT_OF_PARAMETERS, process_results
app = Dash(__name__,
...
app.layout = dbc.Container(dbc.Stack(
    ...
    dcc.Markdown("# Country by Country Tax Reporting analysis",
    ...
    dcc.Upload(
            ...
            children=dcc.Loading(
                type="default",
                children=html.Div(id="loading_text_pdf")
                ),
            style={
                'height': '30px',
                'lineHeight': '30px',
     ...
     dbc.Row([
        dbc.Col([
            dcc.Dropdown(
              options=(
                   [{"value": DEFAULT_METHOD, "label": f"{DEFAULT_METHOD} (default)"}]
                   + [{"value": k, "label":k} for k in DICT_OF_METHODS if k != DEFAULT_METHOD]
                    ),
        ...
        dbc.Col([
            dbc.Button(dcc.Loading(
                type="default",
                children=html.Div(id="run_button_text")
                ),
                id='run_button', n_clicks=0, 
                      outline=True, color="info", class_name="col-12", size="sm"),
            ], width=4,)
     ],
     justify="around"),
     html.Br(),
     dbc.Row(dcc.Markdown("Method parameters"),),
     dbc.Row(
         children=[
             dbc.Col(
                dbc.Fade([
         ...
         dbc.Col([
             dbc.Row([
                 ...
                 dbc.Col(
                    ...
                    dcc.Checklist(
                        options=[{"value": "on_bbox", "label": " Zoom on selected bounding box"},
                    ...
                    width=6),
                 dbc.Col([
                    dbc.Button("Add a new bounding box",
                                id="add_bounding_box",  outline=True, 
                                color="success", class_name="col-12", size="sm"),
                    dbc.Button("Edit the bounding box",
                                id="get_bounding_box",  outline=True, 
                                color="warning", class_name="col-12", size="sm"),
                    ],
                    width=3),
                ]),
             dcc.Graph(id="graph_image", 
     ...
     dbc.Row([dbc.Button("Download current table", id="download_current-button",
                         outline=True, color="secondary"),
              dbc.Button("Download all tables", id="download_all-button",
...
def read_file(pdf_data, parameters,
              method, run_button, file, file_name, 
              selected_page, selected_table, table_data
              ):
    """
    ...
    if file_name is None:
        ...
        results = process_results(results["results"]) # note : other keys are discarded for now
        run_button_name = f"Method '{method}' has run"
        print(run_button_name 
        ...
        possible_tables = [k for k, p in pdf_data["pages"].items() if p == selected_page]
        if len(possible_tables) == 0:
            selected_table = None
        elif selected_table not in possible_tables:
            selected_table = possible_tables[0]
    elif "table_results.data" in triggered_ids: # update pdf_data to save any modification after change of selected_table
    ...
    if ("records" in pdf_data.keys()) and (
        ...
        if selected_table is not None:
            table_data = pdf_data["records"].get(selected_table, ["Run file extraction method with this bounding box"])
            columns = pdf_data["columns"].get(selected_table, ["Instructions for new bounding box"])
        else:
            table_data, columns = [], []
    return pdf_data, file_name, run_button_name, page_slider, selected_page, table_list, selected_table, table_data, columns
...
def plot_image(data, parameters, page, selected_table, graph_options):
    ...
    for idx, bbox in parameters.get("bounding_boxes", {}).items():
        ...
        fig.add_shape(
            ...
            opacity=.2,
            line={"color": "black", 
                  "width": 3 if idx == selected_table else 0
                 },
            )
    return fig
...
def set_parameters(parameters, page_selected, table_selected, figure_annotations,
                    method, pdf_data, get_bounding_box, add_bounding_box,
                   dpi, column_name,
                   ):
    """
    ...
    if "dropdown_method_selector.value" in triggered_ids:
        ...
        i.split(".")[0] in method_parameters
        ...
        for key in method_parameters:
            if key not in parameters.keys():
                parameters[key] = eval(key) # /!\ use copmponent id as parameter in set_parameters
    elif not isinstance(figure_annotations, dict):
        raise PreventUpdate
    elif ("get_bounding_box.n_clicks" in triggered_ids
          or "add_bounding_box.n_clicks" in triggered_ids):    
        bbox = [shape for shape in figure_annotations.get("shapes", [])
                            if shape.get("type", "unknown") == "rect"
                            ]
        if len(bbox) > 0:
            if "get_bounding_box.n_clicks" in triggered_ids and table_selected is not None:
    ...
    return tuple(
        [no_update] * len(layout_options) # hideable parameters
        + [parameters]
        )
...
def download_tables(data, file_name, selected_table,
                     download_current, download_selection):
    """
    Export the tables
    ...
    if "download_current-button.n_clicks" in triggered_ids and selected_table is not None:
        df = pd.DataFrame.from_records(
            data["records"][selected_table]
            )
        return dcc.send_data_frame(df.to_excel, f"{export_name}.xlsx", sheet_name="Table")
    else:
        def to_xlsx(bytes_io):
            writer = pd.ExcelWriter(bytes_io, engine='xlsxwriter')
            for n, records in enumerate(data["records"].values()):
                pd.DataFrame.from_records(records).to_excel(writer, sheet_name=f"Table {n + 1}")
            writer.save() 
