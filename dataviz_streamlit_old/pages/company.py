...
def show_company():
    ...
    *SELECT BOX “COMPANY” (possibly ordered by sector)
    ...
    def create_selection(
        ...
        selected_element_map[selection_colmun_name] = st.selectbox(
            "Select a " + selection_colmun_label,
            element_list,
            index=len(element_list) - 1,
        )
        df_selected_element = df[
            df[selection_colmun_name] == selected_element_map[selection_colmun_name]
        ]
        return df_selected_element

    with st.sidebar:
        df_selected_company = create_selection("mnc", "company")
        df_selected_fiscal_year = create_selection("year", "fiscal year")
    ...
    df_selected_company_per_year = df_selected_company.groupby(["year"]).aggregate(
        {"year": ["count"], "tax_paid": ["sum", "min", "max", "mean"]}
    )
    df_selected_company_per_year.columns = df_selected_company_per_year.columns.map(
        "_".join
    )
    ...
    row[1].markdown(
        """#### line chart showing the evolution of number of report over the time"""
    )
    ...
    fig.layout.xaxis2 = go.layout.XAxis(
        overlaying="x", range=[plot_min, plot_max], showticklabels=False
    )
    fig.add_scatter(
        ...
        xaxis="x2",
        showlegend=False,
        line=dict(dash="dash", color="firebrick", width=5),
    )
    ...
    row[1].line_chart(
        df_per_country_per_year, x="year", y="year_count", color="jur_name"
    )
    ...
    df_fiscal_year_per_company = (
        df.groupby(["mnc"])[columns_of_interest].sum().reset_index()
    )
